/**
 * Color tokens
 * AI-generated tokens should follow this schema for automatic discovery.
 *
 * @ai-token-schema
 * Each token has: name, value, description, category, usage
 */

export const colors = {
  // Primitive palette
  gray: {
    0: "#ffffff",
    50: "#f9fafb",
    100: "#f3f4f6",
    200: "#e5e7eb",
    300: "#d1d5db",
    400: "#9ca3af",
    500: "#6b7280",
    600: "#4b5563",
    700: "#374151",
    800: "#1f2937",
    900: "#111827",
    950: "#030712",
    1000: "#000000",
  },
  blue: {
    50: "#eff6ff",
    100: "#dbeafe",
    200: "#bfdbfe",
    300: "#93c5fd",
    400: "#60a5fa",
    500: "#3b82f6",
    600: "#2563eb",
    700: "#1d4ed8",
    800: "#1e40af",
    900: "#1e3a8a",
    950: "#172554",
  },
  green: {
    50: "#f0fdf4",
    100: "#dcfce7",
    200: "#bbf7d0",
    300: "#86efac",
    400: "#4ade80",
    500: "#22c55e",
    600: "#16a34a",
    700: "#15803d",
    800: "#166534",
    900: "#14532d",
    950: "#052e16",
  },
  red: {
    50: "#fef2f2",
    100: "#fee2e2",
    200: "#fecaca",
    300: "#fca5a5",
    400: "#f87171",
    500: "#ef4444",
    600: "#dc2626",
    700: "#b91c1c",
    800: "#991b1b",
    900: "#7f1d1d",
    950: "#450a0a",
  },
  yellow: {
    50: "#fefce8",
    100: "#fef9c3",
    200: "#fef08a",
    300: "#fde047",
    400: "#facc15",
    500: "#eab308",
    600: "#ca8a04",
    700: "#a16207",
    800: "#854d0e",
    900: "#713f12",
    950: "#422006",
  },
  purple: {
    50: "#faf5ff",
    100: "#f3e8ff",
    200: "#e9d5ff",
    300: "#d8b4fe",
    400: "#c084fc",
    500: "#a855f7",
    600: "#9333ea",
    700: "#7e22ce",
    800: "#6b21a8",
    900: "#581c87",
    950: "#3b0764",
  },
} as const;

/** Semantic color aliases - these should be used in components */
export const semanticColors = {
  background: {
    default: colors.gray[0],
    subtle: colors.gray[50],
    muted: colors.gray[100],
    inverted: colors.gray[900],
  },
  foreground: {
    default: colors.gray[900],
    subtle: colors.gray[600],
    muted: colors.gray[400],
    inverted: colors.gray[0],
  },
  border: {
    default: colors.gray[200],
    subtle: colors.gray[100],
    strong: colors.gray[300],
  },
  primary: {
    default: colors.blue[600],
    hover: colors.blue[700],
    active: colors.blue[800],
    subtle: colors.blue[50],
    foreground: colors.gray[0],
  },
  success: {
    default: colors.green[600],
    subtle: colors.green[50],
    foreground: colors.gray[0],
  },
  error: {
    default: colors.red[600],
    subtle: colors.red[50],
    foreground: colors.gray[0],
  },
  warning: {
    default: colors.yellow[500],
    subtle: colors.yellow[50],
    foreground: colors.gray[900],
  },
  // AI-specific semantic colors
  ai: {
    default: colors.purple[600],
    subtle: colors.purple[50],
    foreground: colors.gray[0],
    accent: colors.purple[400],
  },
} as const;

export type Colors = typeof colors;
export type SemanticColors = typeof semanticColors;
